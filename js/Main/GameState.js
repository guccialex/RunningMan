class GameState {
    ...
    //and it's angle, and my handy math will do the rest
    //for determining its angle and position

    this.cameradistance = 15;


    //these "angles"
    //are really just how far away from the object the camera is
    //in these dimensions

    this.cameraxangle = 0.05;
  ...
  camerafollowmain(){
    ...
    //now get the angle , i assume this works
    this.camera.lookAt(this.myguy.getmesh().position);
  ...
  //might need this later but not now
  /*
  createsetting(){

    //create a new setting and set it as main, and put myguy in it
    this.mainsetting = new Setting(this.scene, this.myguy);

    //put it in the array
    this.settinglist.push(this.mainsetting);
